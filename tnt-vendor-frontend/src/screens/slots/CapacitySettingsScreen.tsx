...
export default function CapacitySettingsScreen({ navigation }: any) {
  ...
  const handleApplyAi = async () => {
    try {
      setApplyingAi(true);
      const res = await slotApi.applyAiSlotAdjustment();
      const data = res.data;
      const lines = data.adjustments.map(
        a => `• Slot #${a.slot_id}: ${a.previous_capacity} → ${a.new_capacity} (${a.action.replace('_', ' ')})`,
      );
      Alert.alert(
        'AI Slot Adjustment',
        data.adjustments_applied === 0
          ? `No changes needed. ${data.reasoning}`
          : `${data.adjustments_applied} slot(s) adjusted.\nRecommended capacity: ${data.recommended_capacity}\n\n${lines.join('\n')}`,
      );
      fetchRules();
    } catch (err: any) {
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <Animated.View style={{ opacity: fadeAnim }}>
        <View style={styles.actionsWrap}>
          <Button title="🤖 Apply AI Recommendation" onPress={handleApplyAi} loading={applyingAi} variant="secondary" size="md" fullWidth style={{ marginBottom: spacing.sm }} />

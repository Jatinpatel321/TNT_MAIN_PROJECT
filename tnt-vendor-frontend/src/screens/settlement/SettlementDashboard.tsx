            ...
            {data?.revenue?.today && (
              <GlassCard padding={16} borderRadius={20} style={{ marginBottom: spacing.md }}>
                ...
                <View style={styles.todayGrid}>
                  ...
                  <View style={[styles.todayCard, { backgroundColor: colors.bgSecondary }]}>
                    <Text style={[styles.todayValue, { color: colors.textPrimary }]}>{formatRupees(data.revenue.today.cash_orders || 0)}</Text>
                    <Text style={[styles.todayLabel, { color: colors.textMuted }]}>Cash</Text>
                  </View>
                  <View style={[styles.todayCard, { backgroundColor: colors.bgSecondary }]}>
          ...
          <Animated.View style={{ opacity: fadeAnim }}>
            {data?.transactions?.summary && (
              <View style={styles.statsRow}>
                ...
                <StatCard value={data.transactions.summary.total_cash || 0} label="Cash" prefix="₹" color={colors.success} style={{ flexBasis: '45%', flexGrow: 1 }} format="currency" />
              </View>
            )}
              ...
              <GlassCard padding={16} borderRadius={20}>
                ...
                {data.settlements.settlements.map((s: any, i: number) => (
                  <View key={i} style={[styles.settRow, { borderBottomColor: colors.borderLight }]}>
                    ...
                    <Text style={[styles.settDetail, { color: colors.textSecondary }]}>Orders: {s.order_count} · Online: {formatRupees(s.online_payments)} · Cash: {formatRupees(s.cash_orders)}</Text>

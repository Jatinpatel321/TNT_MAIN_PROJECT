...
export function QRScannerScreen({navigation}: Props) {
  ...
  // A scanned/pasted payload may be a raw token or a JSON blob
  // ({order_id|group_id, token, type}). Extract the underlying token and
  // whether it belongs to a whole-group pickup.
  const parseScannedQr = (raw: string): {token: string; isGroup: boolean} => {
    const trimmed = raw.trim();
    try {
      const parsed = JSON.parse(trimmed);
      if (parsed && typeof parsed === 'object' && parsed.token) {
        const token = String(parsed.token);
        return {token, isGroup: parsed.type === 'group' || token.startsWith('GRP-')};
      }
    } catch {
      // not JSON — fall through to raw handling
    }
    return {token: trimmed, isGroup: trimmed.startsWith('GRP-')};
  };
  ...
  const handleConfirmPickup = useCallback(async (qrCode: string) => {
    ...
    try {
      setConfirming(true);
      const {token, isGroup} = parseScannedQr(qrCode);
      if (isGroup) {
        const {data} = await vendorApi.confirmGroupPickup(token);
        Vibration.vibrate(200);
        const picked = data?.picked_count ?? 0;
        const total = data?.total_orders ?? 0;
        Alert.alert(
          picked > 0 ? '✅ Group Pickup Confirmed' : 'ℹ️ Nothing Picked Up',
          picked > 0
            ? `${picked} of ${total} group orders handed over.`
            : 'None of the group orders are ready yet.',

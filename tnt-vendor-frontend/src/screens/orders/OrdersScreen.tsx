  ...
  const renderOrderCard = ({ item }: { item: Order }) => {
    ...
    return (
      <GlassCard style={styles.orderCard} padding={20} borderRadius={24} intensity="light">
        ...
        {/* Print Job Details (combined food + stationery bookings) */}
        {item.stationery_jobs?.map((job) => (
          <View key={job.id} style={[styles.itemsRow, { borderTopColor: colors.borderLight }]}>
            <Text style={styles.itemsLabel}>🖨️ Print: </Text>
            <Text style={[styles.itemsList, { color: colors.textSecondary }]} numberOfLines={2}>
              {[
                job.print_type === 'color' ? 'Color' : 'B&W',
                job.paper_size ?? 'A4',
                job.duplex ? 'Double-sided' : 'Single-sided',
                `${job.quantity} ${job.quantity === 1 ? 'copy' : 'copies'}`,
                job.page_range ? `Pages ${job.page_range}` : 'All pages',
                job.notes ? `Note: ${job.notes}` : null,
              ].filter(Boolean).join(' · ')}
            </Text>
          </View>
        ))}

// ─── Premium Logo Upload ────────────────────────────────────────
// Upload business logo with premium design system
...
export default function LogoUploadScreen({ navigation }: any) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await imageUploadApi.uploadLogo(selectedImage, (p) => setUploadProgress(p));
      Alert.alert('Success', 'Logo uploaded', [{ text: 'OK', onPress: () => navigation.goBack() }]);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>Add your business logo to personalize your store</Text>
      </View>
        ...
        <View style={styles.contentSection}>
          {selectedImage ? (
            ...
            <GlassCard padding={20} borderRadius={20}>
              <ImagePicker onImageSelected={handleImageSelected} title="Select Logo" />
            </GlassCard>
          )}

          {selectedImage && (
            <Button title="Upload Logo" onPress={handleUpload} loading={uploading} variant="primary" size="lg" fullWidth style={{ marginTop: spacing.md }} />
          )}

          <GlassCard padding={16} borderRadius={18} style={{ marginTop: spacing.md }}>
            <Text style={styles.guideTitle}>📋 Guidelines</Text>
            {['512x512 pixels recommended', 'JPEG, PNG, or WebP', 'Max 5MB', 'Square image for best results', 'High resolution'].map((g, i) => (
          ...
          <GlassCard padding={16} borderRadius={18} style={{ marginTop: spacing.sm, backgroundColor: colors.infoPale }}>
            <Text style={styles.helpText}>💡 Your logo is displayed on your store page and customer communications. Make it clear and professional.</Text>
          </GlassCard>
        </View>

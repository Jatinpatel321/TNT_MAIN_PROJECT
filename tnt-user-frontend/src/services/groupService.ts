...
export interface GroupPickupQr {
  group_id: number;
  qr_code: string;
  total_orders: number;
  ready_orders: number;
...
/**
 * Fetch the single combined-pickup QR for a placed group order.
 * Any group member may fetch it; the group presents one code at the stall.
 */
export async function getGroupPickupQr(groupId: number): Promise<GroupPickupQr> {
  const res = await apiClient.get<GroupPickupQr>(`/groups/${groupId}/pickup-qr`);
  return res.data;
}

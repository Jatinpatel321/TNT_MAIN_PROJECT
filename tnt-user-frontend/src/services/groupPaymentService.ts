...
// ── Group order tracking (per-member orders + payment state) ──────────────

export type MemberOrderInfo = {
  member_id: number;
  order_id: number;
  amount: number;
  // Populated by the backend at order placement (place_group_order).
  payable_amount: number;
  payment_status: string;
  payment_id: number;
};

export type StoredGroupOrderData = {
  groupId: number;
  orders: MemberOrderInfo[];
  total_amount: number;
};

function groupOrderStorageKey(groupId: number): string {
  return `group_order_data_${groupId}`;
}

export async function storeGroupOrderData(
  groupId: number,
  data: StoredGroupOrderData,
): Promise<void> {
  await AsyncStorage.setItem(groupOrderStorageKey(groupId), JSON.stringify(data));
}

export async function getStoredGroupOrderData(
  groupId: number,
): Promise<StoredGroupOrderData | null> {
  const raw = await AsyncStorage.getItem(groupOrderStorageKey(groupId));
  if (!raw) return null;
  try {
    return JSON.parse(raw) as StoredGroupOrderData;
  } catch {
    return null;
  }
}
...
export async function clearGroupOrderData(groupId: number): Promise<void> {
  await AsyncStorage.removeItem(groupOrderStorageKey(groupId));
}

/**
 * Fetch paid/unpaid status for a batch of order IDs (group order tracking).
 * Backed by `POST /orders/payment-status`.
 */
export async function batchCheckPaymentStatus(
  ...
  const statuses = (res.data?.statuses ?? {}) as Record<string, 'paid' | 'unpaid'>;
  const map: Record<number, 'paid' | 'unpaid'> = {};
  for (const [oid, status] of Object.entries(statuses)) {
    map[Number(oid)] = status;
  }
  return map;
}

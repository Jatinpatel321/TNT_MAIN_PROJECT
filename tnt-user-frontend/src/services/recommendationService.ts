...
export type PersonalizedOffersResponse = {
  offers: PersonalizedOffer[];
  generated_at: string;
  count: number;
};

export async function getPersonalizedOffers(limit = 10): Promise<PersonalizedOffersResponse> {
  const res = await apiClient.get('/recommendations/offers/personalized', {
    headers: await authHeaders(),
    params: { limit },
  });
  return res.data as PersonalizedOffersResponse;
}

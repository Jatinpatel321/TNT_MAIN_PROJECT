...
import { addFavoriteVendor, getFavoriteVendors, removeFavoriteVendor } from '../../services/profileService';
...
export function VendorListScreen({ navigation, route }: Props) {
  ...
  const loadFavorites = async () => {
    try {
      const favs = await getFavoriteVendors();
      setFavoriteIds(new Set(favs.map((f) => f.vendor_id)));
    } catch {
      // favourites are a non-critical enhancement — ignore load failures
    }
  };

  const onToggleFavorite = async (vendorId: number) => {
    ...
    try {
      if (isFav) await removeFavoriteVendor(vendorId);
      else await addFavoriteVendor(vendorId);
    } catch (e) {
      // revert on failure
      setFavoriteIds((prev) => {
        const next = new Set(prev);

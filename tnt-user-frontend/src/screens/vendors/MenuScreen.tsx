...
import { addFavoriteMenuItem, getFavoriteMenuItems, removeFavoriteMenuItem } from '../../services/profileService';
...
export function MenuScreen({ route, navigation }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const favs = await getFavoriteMenuItems();
        setFavoriteIds(new Set(favs.map((f) => f.menu_item_id)));
      } catch {
        // favourites are a non-critical enhancement — ignore load failures
      }
    })();
  }, []);

  const onToggleFavorite = async (menuItemId: number) => {
    ...
    try {
      if (isFav) await removeFavoriteMenuItem(menuItemId);
      else await addFavoriteMenuItem(menuItemId);
    } catch (e) {
      setFavoriteIds((prev) => {
        const next = new Set(prev);
        if (isFav) next.add(menuItemId);
        else next.delete(menuItemId);
        return next;
      });
      Alert.alert('Failed to update favourites', toApiError(e).message);
    }
  };

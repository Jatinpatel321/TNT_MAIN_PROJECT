...
import { useGroupCartWebSocket } from '../../hooks/useGroupCartWebSocket';
...
import type { GroupPickupQr } from '../../services/groupService';
...
export function GroupDetailScreen() {
  ...
  // single combined-pickup QR for the whole group order
  const [pickupQr, setPickupQr] = useState<GroupPickupQr | null>(null);
  ...
  const reload = useCallback(async () => {
    try {
      ...
      // Fetch the single combined-pickup QR once the group order is placed.
      if (detail?.status === 'ordered' || detail?.status === 'completed') {
        try {
          setPickupQr(await getGroupPickupQr(groupId));
        } catch {
          setPickupQr(null);
        }
      } else {
  ...
  // ── Real-time sync: refresh on any group event instead of polling ─────
  const {isConnected: wsConnected} = useGroupCartWebSocket(
    groupId,
    token,
    useCallback(
      (evt) => {
        // Any group change → refresh the shared cart / members / splits.
        reload();
        // Payment-related events also refresh per-member payment status.
        if (evt.event === 'payment_recorded' || evt.event === 'order_placed') {
          checkPayments();
        }
      },
      [reload, checkPayments],
    ),
  );

  // Payment-status poller — now a FALLBACK, only active while the realtime
  // WebSocket is disconnected (preserves behaviour on flaky connections).
  useFocusEffect(
    useCallback(() => {
      if (
        !wsConnected &&
    ...
    }, [wsConnected, group?.status, groupOrders, checkPayments]),
  );
    ...
    <Screen scroll>
      ...
      {group?.status === 'ordered' && (
        <>
          ...
          {/* Single combined-pickup QR for the whole group */}
          {pickupQr && (
            <RoundedCard>
              <View style={styles.sectionRow}>
                <Text style={styles.sectionTitle}>Group Pickup QR</Text>
                <Text style={styles.qrReadyCount}>
                  {pickupQr.ready_orders + pickupQr.picked_orders}/{pickupQr.total_orders} ready
                </Text>
              </View>
              <View style={styles.groupQrWrap}>
                <QRCode
                  value={JSON.stringify({ group_id: pickupQr.group_id, token: pickupQr.qr_code, type: 'group' })}
                  size={200}
                />
              </View>
              <Text style={styles.groupQrHint}>
                Show this single code at the counter — the vendor scans it once to hand over
                every member's order together.
              </Text>
            </RoundedCard>
          )}
...
const styles = StyleSheet.create({
  ...
  qrReadyCount: { fontSize: 12, fontWeight: '700', color: '#6C63FF' },
  groupQrWrap: { alignItems: 'center', justifyContent: 'center', paddingVertical: 16 },
  groupQrHint: { fontSize: 13, color: '#4B5563', textAlign: 'center' },

  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      if (addStationery && stationeryItems && stationeryItems.length > 0) {
        // Combined booking: forward the real food cart items alongside the
        // stationery items so both reach the backend in a single order.
        const foodItems = (cart?.items ?? []).map((ci) => ({

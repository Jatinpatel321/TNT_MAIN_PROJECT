...
import { getFavoriteMenuItems, getFavoriteVendors } from '../../services/profileService';
import type { FavoriteMenuItem, FavoriteVendor } from '../../services/profileService';
...
export function ProfileScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [p, myOrders, food, stationery, rp, favVendors, favItems] = await Promise.all([
          getProfile(),
          getMyOrders(),
          getVendors('food'),
          getVendors('stationery'),
          getPoints().catch(() => null),
          getFavoriteVendors().catch(() => []),
          getFavoriteMenuItems().catch(() => []),
        ]);
        setProfile(p);
        setOrders(myOrders);
        setRewardPoints(rp);
  ...
  return (
    <Screen scroll>
      ...
      {favoriteVendors.length > 0 && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Favourite Stalls</Text>
          {favoriteVendors.map((fv) => (
            <Pressable
              key={fv.vendor_id}
              style={styles.favoriteRow}
              onPress={() => navigation.navigate('Menu' as any, { vendorId: fv.vendor_id, vendorName: fv.vendor_name })}
            >
              <MaterialCommunityIcons name="store" size={18} color="#6C63FF" />
              <Text style={styles.favoriteRowText}>{fv.vendor_name ?? `Vendor #${fv.vendor_id}`}</Text>
      ...
      {favoriteMenuItems.length > 0 && (
        <View style={styles.section}>
          ...
          {favoriteMenuItems.map((fi) => (
            ...
            >
              <MaterialCommunityIcons name="food" size={18} color="#6C63FF" />
              <Text style={styles.favoriteRowText}>{fi.name ?? `Item #${fi.menu_item_id}`}</Text>
              <MaterialCommunityIcons name="chevron-right" size={18} color="#9CA3AF" />
            </Pressable>
          ))}
        </View>

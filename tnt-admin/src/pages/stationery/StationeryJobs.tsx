...
import { formatDateTime, formatTimeAgo, formatRupees } from '../../utils/format';
  ...
  const columns: ColumnDef<PrintJob, unknown>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="max-w-[160px]">
          <p className="text-sm text-[#F1F0FF] truncate">{row.original.service_name}</p>
          {row.original.file_url && (
            <a
              href={row.original.file_url}
              target="_blank"
              rel="noreferrer"
              className="text-xs text-[#9B9BC4] hover:text-[#E85D24] truncate block"
            >
              {row.original.file_url.split('/').pop()}
            </a>
          )}
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-xs text-[#9B9BC4] space-y-0.5">
            <div>
              <span className="capitalize font-mono">{job.print_type === 'color' ? 'Color' : 'B&W'}</span>
              {' · '}
              <span className="font-mono">{job.paper_size}</span>
              {' · '}
              <span className="font-mono">{job.duplex ? 'Duplex' : 'Simplex'}</span>
            </div>
            <div className="font-mono">{job.quantity} {job.quantity === 1 ? 'copy' : 'copies'} · {job.page_range || 'All pages'}</div>
            {job.notes && <div className="italic truncate max-w-[180px]">"{job.notes}"</div>}
          </div>
        );
      },
    },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          <p className="font-mono text-sm text-[#F1F0FF]">{formatRupees(row.original.total_amount)}</p>

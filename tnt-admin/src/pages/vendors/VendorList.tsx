...
import { Search, Store, CheckCircle, XCircle, Eye, RefreshCw, Plus } from 'lucide-react';
...
export default function VendorList() {
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'food' | 'stationery' | 'mixed'>('all');
  ...
  const handleBulkApprove = async () => {
    if (selectedIds.size === 0) { toast.error('Select at least one vendor'); return; }
    setBulkApproving(true);
    try {
      const res = await adminApi.bulkApproveVendors(Array.from(selectedIds));
      toast.success(`Approved ${res.data?.approved_count ?? selectedIds.size} vendors`);
  ...
  const filteredVendors = vendors.filter(v => {
    ...
    const matchSearch = !searchQuery ||
      ...
      (v.stall || '').toLowerCase().includes(q) ||
      (v.vendor_meta?.business_name || '').toLowerCase().includes(q) ||
  ...
  return (
    <div className="space-y-5">
      {showCreate && (
        <VendorFormModal onClose={() => setShowCreate(false)} onSaved={fetchVendors} />
      )}
        ...
        <>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              ...
              <input
                type="text"
                placeholder="Search name, phone, location, stall, type..."
                value={searchQuery}
            ...
            <button onClick={fetchVendors} aria-label="Refresh vendors" className="btn-ghost">
      ...
      {tab === 'pending' && (
        <div>
          {pendingVendors.length > 0 && (
            <div className="flex items-center justify-between mb-4 p-3 bg-[#F3F5F9] border border-[#E5E7EB] rounded-xl">
              <label className="flex items-center gap-2 text-sm text-[#374151]">
                <input
                  type="checkbox"
                  checked={selectedIds.size > 0 && selectedIds.size === pendingVendors.length}
                  onChange={(e) =>
                    setSelectedIds(e.target.checked ? new Set(pendingVendors.map(v => v.id)) : new Set())
                  }
                />

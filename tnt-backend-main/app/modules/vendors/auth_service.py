...
def logout_vendor(refresh_token: str | None) -> None:
    """Log a vendor out.

    Vendor access tokens are stateless JWTs and expire on their own. If the
    client supplies its refresh token we revoke it immediately by consuming its
    JTI, so it can never be exchanged again. A missing/invalid refresh token is
    treated as an already-completed logout (no error).
    """

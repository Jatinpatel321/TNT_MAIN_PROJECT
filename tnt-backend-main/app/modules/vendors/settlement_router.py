...
def get_revenue_summary(
    ...
    """Get revenue summary with wallet, today's revenue, and online/refund breakdown."""
...
def get_transactions(
    ...
    """Get transaction history with online/refund breakdown."""
    db_user = db.query(User).filter(User.phone == user["phone"]).first()
    if not db_user:
        raise HTTPException(status_code=404, detail="User not found")
...
def get_daily_revenue(
    ...
    """Get daily revenue breakdown (online/refunds/net) for last N days."""
    db_user = db.query(User).filter(User.phone == user["phone"]).first()
    if not db_user:
        raise HTTPException(status_code=404, detail="User not found")

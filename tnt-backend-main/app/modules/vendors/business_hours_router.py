...
@router.put("/holidays", summary="Update holiday list")
def api_update_holidays(
    data: dict[str, Any],
    db: Session = Depends(get_db),
    user=Depends(get_current_user),
) -> dict[str, Any]:
    """Replace the vendor's holiday list.

    Body: ``{"holidays": [{"date": "2026-12-25", "reason": "Christmas"}, ...]}``
    """
    db_user = db.query(User).filter(User.phone == user["phone"]).first()
    if not db_user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    holidays = data.get("holidays")
    if holidays is None or not isinstance(holidays, list):
        raise HTTPException(status_code=400, detail="'holidays' must be a list")

    try:
        return update_vendor_business_hours(
            vendor_id=db_user.id, holidays=holidays, db=db
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@router.put("/pickup-instructions", summary="Update pickup instructions")
def api_update_pickup_instructions(
    data: dict[str, Any],
    db: Session = Depends(get_db),
    user=Depends(get_current_user),
) -> dict[str, Any]:
    """Update the free-text pickup instructions shown to customers.

    Body: ``{"pickup_instructions": "Collect from counter 2 ..."}``
    """
    db_user = db.query(User).filter(User.phone == user["phone"]).first()
    if not db_user:
        raise HTTPException(status_code=404, detail="User not found")

    instructions = data.get("pickup_instructions")
    if instructions is None or not isinstance(instructions, str):
        raise HTTPException(status_code=400, detail="'pickup_instructions' must be a string")

    try:
        return update_vendor_business_hours(
            vendor_id=db_user.id, pickup_instructions=instructions, db=db
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

...
class AIIntelligenceService:
    ...
    def apply_slot_adjustments(self, vendor_id: int) -> "SlotAdjustmentResponse":
        """Connect the (previously dead) slot-adjustment planner to real slot capacity.

        Reads the planner's capacity recommendation + adjustment signals and
        actually applies them:

        * ``underutilized_slot`` → reduce that slot's ``max_orders`` toward the
          AI-recommended capacity (never below ``current_orders`` or the floor),
          and persist an AI-managed ``SlotCapacityRule`` for that window so
          future slots inherit the cap via the existing ``_apply_capacity_rules``
          booking path.
        * ``peak_hour_detected`` → raise ``max_orders`` on the vendor's upcoming
          slots in the peak-hour window up to the recommended capacity so more
          students can book during the rush.

        Returns a structured report of everything that was changed. Idempotent —
        re-running when nothing needs changing yields an empty ``adjustments``
        list.
        """
        ...
        AI_RULE_PREFIX = "AI Auto-Adjustment"

        recommendation = self.slot_planner.get_capacity_recommendation(vendor_id)
        recommended_capacity = int(recommendation["recommended_capacity"])
        signals = self.slot_planner.get_slot_adjustment_signals(vendor_id)
        ...
        for signal in signals:
            ...
            if sig_type == "underutilized_slot":
                slot_id = signal.get("slot_id")
                slot = self.db.query(Slot).filter(Slot.id == slot_id).first()
                if not slot:
                    continue
                target = max(recommended_capacity, slot.current_orders or 0, MIN_CAPACITY_FLOOR)
                # Only reduce underutilized slots — never inflate them here.
                new_capacity = min(slot.max_orders, target)
                if new_capacity != slot.max_orders:
                    previous = slot.max_orders
                    slot.max_orders = new_capacity
                    slot.dynamic_capacity = new_capacity
                    slot.capacity_notes = (
                        f"AI reduced capacity {previous}→{new_capacity} "
                        f"(underutilized; {recommendation['reasoning']})"
                    )
                    adjustments.append({
                    ...
                    # Persist a capacity rule so FUTURE slots in this window
                    # inherit the cap through the existing booking path.
                    window_start = slot.start_time.hour
                    window_end = min(window_start + 1, 23)
                    rule_name = f"{AI_RULE_PREFIX} (Underutilized {window_start:02d}:00)"
                    existing_rules = get_slot_capacity_rules(vendor_id, self.db)
                    match = next((r for r in existing_rules if r.rule_name == rule_name), None)
                    rule_data = {
                        "rule_name": rule_name,
                    ...
                    if match:
                        update_slot_capacity_rule(match.id, vendor_id, {"base_capacity": new_capacity}, self.db)
                    else:
                        create_slot_capacity_rule(vendor_id, rule_data, self.db)

            elif sig_type == "peak_hour_detected":
                # Raise capacity on upcoming slots in the current peak hour so
                # the rush can be absorbed.
                peak_hour = now.hour
                peak_slots = self.db.query(Slot).filter(
                    Slot.vendor_id == vendor_id,
                    Slot.start_time >= now,
                ).all()
                for slot in peak_slots:
                    if slot.start_time.hour != peak_hour:
                        continue
                    target = max(recommended_capacity, slot.max_orders)
                    if target != slot.max_orders:
                        previous = slot.max_orders
                        slot.max_orders = target
                        slot.dynamic_capacity = target
                        slot.is_peak_hour = True
                        slot.capacity_notes = (
                            f"AI raised capacity {previous}→{target} "
                            f"(peak hour; {recommendation['reasoning']})"
                        )
                        adjustments.append({
    ...
    def get_ai_recommendations(self, user_id: int) -> Dict[str, Any]:
        """
        Generate personalized recommendations based on order frequency,
        vendor load, time of day, and popularity.
        """
        from app.modules.orders.model import Order, OrderItem
        from app.modules.menu.model import MenuItem
        from app.modules.users.model import User as Vendor
        from app.modules.slots.model import Slot
        from sqlalchemy import func
        ...
        # 1. Frequent orders
        frequent_orders_query = self.db.query(
            MenuItem.id, MenuItem.name, func.count(Order.id).label("order_count")
        ...
        frequent_orders = [{"item_id": r.id, "name": r.name, "frequency": r.order_count} for r in frequent_orders_query]
        
        # 2. Vendors Ranked & Favorite vendors (Frequency + Load)
        vendors_query = self.db.query(
            Vendor.id, Vendor.name, func.count(Order.id).label("order_count")
        ...
        vendors_ranked = [{"vendor_id": r.id, "name": r.name, "score": r.order_count * 10} for r in vendors_query]
        
        # 3. Recommended & Popular items (Campus wide + personalized)
        popular_items_query = self.db.query(
            MenuItem.id, MenuItem.name, func.count(Order.id).label("popularity")
        ).join(OrderItem, OrderItem.menu_item_id == MenuItem.id)\
         .join(Order, Order.id == OrderItem.order_id)\
         .group_by(MenuItem.id, MenuItem.name)\
         .order_by(func.count(Order.id).desc())\
        ...
        recommended_items = [{"item_id": r.id, "name": r.name, "reason": "Trending Campus Item", "score": r.popularity} for r in popular_items_query]
        
        # 4. Best Slots (Based on vendor load and time of day)
        best_slots_query = self.db.query(
            Slot.id, Slot.start_time, Slot.end_time, Slot.vendor_id, Slot.current_orders, Slot.max_orders
        ).filter(Slot.current_orders < Slot.max_orders)\
         .order_by(Slot.current_orders.asc())\
         .limit(3).all()
         
        best_slots = [{"slot_id": r.id, "time": f"{r.start_time}-{r.end_time}", "vendor_id": r.vendor_id, "load_status": "low"} for r in best_slots_query]
        
        return {
            "recommended_items": recommended_items,
            "frequent_orders": frequent_orders,
            "best_slots": best_slots,
            "vendors_ranked": vendors_ranked
        }

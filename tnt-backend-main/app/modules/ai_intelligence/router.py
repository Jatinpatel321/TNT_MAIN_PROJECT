...
@router.post("/apply-slot-adjustment", response_model=SlotAdjustmentResponse)
def apply_slot_adjustment(
    db: Session = Depends(get_db),
    user=Depends(require_role("vendor")),
) -> SlotAdjustmentResponse:
    """Apply AI slot-capacity adjustments for the authenticated vendor.

    Connects the slot-adjustment planner to real slot capacity: reduces
    underutilized slots and raises capacity on peak-hour slots based on the
    7-day demand recommendation. Returns a report of every change made.
    """
    ...
    service = AIIntelligenceService(db)
    return service.apply_slot_adjustments(db_user.id)

...
class GroupPaymentService:
    ...
    def record_payment(
        ...
        # Real-time push so every member's UI reflects the payment immediately
        # (this is what lets the frontend treat the 8s poller as a fallback).
        try:
            from app.core.group_events import publish_group_event
            publish_group_event(group_id, "payment_recorded", {
                "user_id": user_id,

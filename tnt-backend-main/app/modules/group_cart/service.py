...
class GroupCartService:
    ...
    def _notify_group(
        ...
        event: str = "group_updated",
    ) -> None:
        # ── Real-time push: notify every connected member to refresh (this is
        # what replaces client-side polling). Best-effort — never blocks the
        # DB notification path below.
        try:
            from app.core.group_events import publish_group_event
            publish_group_event(group_id, event, {"title": title, "message": message})
        except Exception:
            pass

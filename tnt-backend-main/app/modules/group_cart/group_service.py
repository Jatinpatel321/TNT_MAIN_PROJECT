...
def get_group_pickup_qr(group_id: int, user: dict, db: Session) -> dict:
    """Return the single combined-pickup QR for a placed group order.

    Any member may fetch it. Also reports per-order readiness so the UI can
    show how much of the group is collectable.
    """
    from app.modules.orders.model import Order, OrderStatus
    from app.modules.orders.qr_service import generate_group_qr_code

    _svc(db).get_group(group_id, user["id"])  # membership / access check

    try:
        qr_code = generate_group_qr_code(group_id, db)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))

    orders = db.query(Order).filter(Order.group_id == group_id).all()
    ready_states = {OrderStatus.READY, OrderStatus.READY_FOR_PICKUP}
    ready = sum(1 for o in orders if o.status in ready_states)
    picked = sum(1 for o in orders if o.status == OrderStatus.PICKED)
    return {
        "group_id": group_id,
        "qr_code": qr_code,
        ...
        "all_ready": len(orders) > 0 and ready + picked == len(orders),
    }


def confirm_group_pickup(qr_code: str, vendor: dict, db: Session) -> dict:
    """Vendor-side: confirm pickup for a whole group from one QR scan."""
    from app.modules.orders.qr_service import confirm_group_pickup as _confirm

    result = _confirm(qr_code, vendor["id"], db)

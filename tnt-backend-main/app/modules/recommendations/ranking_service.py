...
class RecommendationRankingService:
    ...
    # ── Personalized offers (wires rank_offers to real candidates) ──────────

    # Campus peak windows (start_hour, end_hour) — mirrors AIIntelligenceService.
    _PEAK_WINDOWS = [(8, 10), (12, 14), (19, 21)]

    def _is_peak_now(self, hour: int) -> bool:
        return any(start <= hour < end for start, end in self._PEAK_WINDOWS)

    def get_personalized_offers(self, user_id: int, limit: int = 10) -> Dict[str, Any]:
        """Build & rank real, currently-valid offers for a user.

        Connects the previously-dead ``rank_offers`` to actual ``VendorOffer``
        candidates and layers personalization factors on top of the SVD/affinity
        base score:

        * order history / affinity  — via ``rank_offers``
        * loyalty                   — repeat-purchase count with the vendor
        * vendor preference         — user's favourited vendors (Phase 1)
        * time of day / rush        — off-peak offers surfaced during peak hours
        * discount depth            — larger effective discount ranks higher
        """
        from app.modules.vendors.retention_models import VendorOffer, DiscountCampaign, OfferType
        from app.modules.users.favorites_model import UserFavoriteVendor
        from app.modules.users.model import User

        now = utcnow_naive()
        current_hour = now.hour

        # 1. Real candidates: active, unexpired, targeted at this user or everyone.
        candidates: List[VendorOffer] = (
            self.db.query(VendorOffer)
            .filter(
                VendorOffer.is_active == True,  # noqa: E712
                (VendorOffer.expires_at.is_(None)) | (VendorOffer.expires_at > now),
                (VendorOffer.target_user_id.is_(None)) | (VendorOffer.target_user_id == user_id),
            )
            .all()
        )
        if not candidates:
            return {"offers": [], "generated_at": now.isoformat(), "count": 0}

        # 2. Map to the dict shape rank_offers expects, keep offer metadata.
        offer_dicts: List[Dict[str, Any]] = []
        by_id: Dict[int, VendorOffer] = {}
        ...
        # 3. Base affinity ranking (SVD or heuristic).
        base_ranked = self.rank_offers(user_id, offer_dicts)

        # 4. Personalization signals (batch-loaded).
        fav_vendor_ids = {
            row.vendor_id for row in
            self.db.query(UserFavoriteVendor.vendor_id).filter(UserFavoriteVendor.user_id == user_id).all()
        }
        # Loyalty: completed order counts per vendor for this user.
        loyalty_rows = (
        ...
        for ranked in base_ranked:
            off = by_id.get(ranked.get("offer_id"))
            if off is None:
                continue
            vendor_id = off.vendor_id
            reasons: List[str] = []
            ...
            # Loyalty boost — repeat customers value a vendor's deals more.
            order_count = int(loyalty_by_vendor.get(vendor_id, 0))
            if order_count > 0:
                loyalty_boost = min(order_count, 10) * 1.5
                score += loyalty_boost
                if order_count >= 3:
                    reasons.append(f"You've ordered here {order_count} times")

            # Vendor preference — favourited vendors rank higher.
            if vendor_id in fav_vendor_ids:
                score += 8.0
                reasons.append("One of your favourite stalls")

            # Time of day / rush — surface off-peak deals during peak crunch.
            campaign = None
            if off.campaign_id:
                campaign = self.db.query(DiscountCampaign).filter(DiscountCampaign.id == off.campaign_id).first()
            if campaign and campaign.is_off_peak and campaign.off_peak_start is not None and campaign.off_peak_end is not None:
                in_offpeak_window = campaign.off_peak_start <= current_hour < campaign.off_peak_end
                if in_offpeak_window:
                    score += 6.0
                    reasons.append("Off-peak deal active right now")
                elif self._is_peak_now(current_hour):
                    score += 3.0
                    reasons.append("Beat the rush — off-peak savings")

            # Discount depth — deeper effective discounts rank a little higher.
            if (off.discount_type == OfferType.DISCOUNT_PERCENTAGE or
                    getattr(off.discount_type, "value", off.discount_type) == "discount_percentage"):
                if (off.discount_value or 0) >= 20:
                    score += 2.0
                    reasons.append(f"{int(off.discount_value)}% off")
            if not reasons:
            ...
            results.append({
                ...
                "method": ranked.get("method", "heuristic"),
                "reason": ", ".join(reasons),
            })

        results.sort(key=lambda x: x["score"], reverse=True)
        results = results[:limit]
        return {"offers": results, "generated_at": now.isoformat(), "count": len(results)}